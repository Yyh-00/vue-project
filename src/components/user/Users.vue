<template>
  <div>
    ...
    <el-card>
     ...
     <!-- 分配权限的对话框 -->
     <el-dialog
      title="分配权限" :visible.sync="setRolesDialogVisible" width="50%" @close='setRolesClose'>
      <div>
...
export default {
  ...
  methods: {
    ...
    async allotRoles() {
      ...
      const { data: res } = await this.$http.put(`users/${this.usersInfo.id}/role`, { rid: this.selectedRoleId })
      console.log(res)
      if (res.meta.status !== 200) {
